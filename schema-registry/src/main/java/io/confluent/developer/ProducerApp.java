...
import io.confluent.developer.avro.Purchase.Builder;
...
//        producerConfigs.put("basic.auth.user.info", "<Replace this with basic.auth.user.info value from confluent.properties>");
        ...
        try (final Producer<String, Purchase> producer = new KafkaProducer<>(producerConfigs)) {
            ...
            String topic = "purchases";  // purchases-key, purchases-value
            List<Purchase> purchaseEvents = new ArrayList<>();

            Purchase purchase = new Purchase();
            purchase.setCustomerId("vandelay");
            purchase.setEmployeeId("kramer");
            purchase.setTotalCost(random.nextDouble() * random.nextInt(100));
            purchase.setItem(items.get(random.nextInt(3)));

            Purchase purchaseII = new Purchase();
            purchaseII.setCustomerId("vandelay");
            purchaseII.setEmployeeId("johnson");
            purchaseII.setTotalCost(random.nextDouble() * random.nextInt(100));
            purchaseII.setItem(items.get(random.nextInt(3)));
